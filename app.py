...
if page == "Disease Prediction":
    ...
    if st.button("Predict"):
        ...
        try:
            diagnosis_response = ask_medical_question(
                f"Patient profile: {profile}. Symptoms: {symptom_list}. "
                f"Only return the single most likely disease and a one-line description. "
                f"Format: Disease: <name> | Description: <one-line>. Do not list multiple diseases."
            )
            ...
            treatment_plan = generate_treatment_plan(diagnosis_response, profile)
            ...
            st.session_state.prediction_result = diagnosis_response.strip().rstrip('. ,\n') + "."
    ...
    if st.session_state.prediction_result:
        highlighted = st.session_state.prediction_result.replace("Disease:", "**🦠 Disease:**").replace("| Description:", "\n**📋 Description:**")
        st.success(highlighted)
